...
if (PanelClass) {
  ...
  proto.identifySteps = function() {
    ...
    const byKind = { tv:["Power","Volume +","Mute"], fan:["Power","Speed","Oscillate / Swing"], projector:["Power","Input / Source","Menu"], speaker:["Power","Volume +","Mute"] };
  ...
  proto.renderCaptureRecognition = function(cap) {
    ...
    const hintText = hint?.protocol ? `<div style="margin-top:6px">${this.tr("Nhận dạng sơ bộ", "Preliminary")}: <b>${this.esc(hint.brand ? `${hint.brand} ${hint.protocol}` : hint.protocol)}</b> ${Number(hint.confidence || 0)}%</div>` : "";
  ...
  proto.render = function(...args) {
    ...
    queueMicrotask(() => {
      ...
      if (!root) return;
      const recognitionIds = new Set((this._identifyResult?.candidates || []).map(row=>row?.candidate).filter(c=>c?.recognition_only&&c?.id).map(c=>String(c.id)));
      root.querySelectorAll("[data-identify-add]").forEach(button => { if (recognitionIds.has(String(button.dataset.identifyAdd || ""))) button.remove(); });
      ...
      if (actionBar && analyze && analyze.parentElement !== actionBar) { analyze.remove(); actionBar.appendChild(analyze); }
      ...
      if (oldSearch && oldSearch.dataset.hanjooV065 !== "1") {
        const search = oldSearch.cloneNode(true); search.dataset.hanjooV065 = "1"; oldSearch.replaceWith(search);
