...
async def _native_rows(client: HanJooCoreClient, captures: list[dict[str, Any]], kind_hint: str) -> list[dict[str, Any]]:
    ...
    # Do NOT probe 3 long A/C captures concurrently. The sidecar intentionally
    # uses native child decoders and broad public protocol coverage. Parallel
    # probe-all calls can briefly multiply memory/CPU and previously caused the
    # sidecar container process to be killed on smaller HA hosts.
...
async def _fuse_with_native_ac(self: HanJooCoreClient, captures: list[dict[str, Any]], *, kind_hint: str = "auto", candidates=None, profiles=None) -> dict[str, Any]:
    ...
    try:
        ...
        # A transient add-on/Brain restart must not surface as a fatal UI error.
        # Keep the recognition session intact and return any strong local decoder
        # evidence already collected. The next Analyze press can use Brain again.
