...
def _canonical_state(value: Any) -> dict[str, Any]:
    ...
    if isinstance(mode, (int, float)):
        state["mode"] = {-1: "off", 0: "auto", 1: "cool", 2: "heat", 3: "dry", 4: "fan_only"}.get(int(mode), mode)
    if state.get("fan") is None:
        fan = state.get("fanspeed")
        if isinstance(fan, (int, float)):
            state["fan"] = {0: "auto", 1: "min", 2: "low", 3: "medium", 4: "high", 5: "max", 6: "medium_high"}.get(int(fan), fan)
...
def _agreement(decoded: dict[str, Any], expected: dict[str, Any]) -> float:
    keys = [key for key, value in expected.items() if value is not None and key in {"power", "mode", "temp", "fan", "swing"}]
    if not keys:
        return 1.0
    score = 0.0
    for key in keys:
...
async def _native_ac_candidates(client: HanJooCoreClient, captures: list[dict[str, Any]], kind_hint: str) -> list[dict[str, Any]]:
    if len(captures) < 2:
        return []
    guided_ac = kind_hint in {"air_conditioner", "climate"} or any(
        any(key in (capture.get("expected") or {}) for key in ("temp", "mode")) for capture in captures
    )
    if not guided_ac:
        return []
    ...
    for key, row in groups.items():
        ...
        confidence = round(52 * valid_ratio + 31 * semantic + (8 if row["structured"] else 0) + (6 if row["native"] else 0) + (3 if distinct >= 2 else 0))
        confidence = max(0, min(100, confidence))
        safe = total >= 3 and matched >= 3 and valid_ratio >= 0.99 and semantic >= 0.84 and distinct >= 2 and confidence >= 90
    ...
    output.sort(key=lambda item: (-int(bool(item.get("_core_recommended"))), -int(item.get("confidence") or 0), -int(item.get("matched_captures") or 0)))
    return output[:24]
