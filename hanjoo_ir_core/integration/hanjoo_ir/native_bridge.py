...
ranking/recommendation.  This module only translates strong, repeated native A/C
decodes into the same candidate/result shape used by the Manager.
"""
...
def _brand(protocol: str) -> str | None:
    p = protocol.upper()
    for token, brand in (
        ("DAIKIN", "Daikin"), ("PANASONIC", "Panasonic"),
        ("MITSUBISHI", "Mitsubishi"), ("FUJITSU", "Fujitsu"),
        ("HITACHI", "Hitachi"), ("SAMSUNG", "Samsung"),
        ("TOSHIBA", "Toshiba"), ("GREE", "Gree"), ("MIDEA", "Midea"),
        ("HAIER", "Haier"), ("SHARP", "Sharp"), ("SANYO", "Sanyo"),
        ("KELVINATOR", "Kelvinator"), ("WHIRLPOOL", "Whirlpool"),
        ("AIRWELL", "Airwell"), ("VESTEL", "Vestel"), ("ELECTRA", "Electra"),
    ):
        if token in p:
            return brand
    return None
...
def _canonical(match: dict[str, Any]) -> dict[str, Any]:
    ...
    mode = raw.get("mode")
    fan = raw.get("fanspeed")
    swingv = raw.get("swingv")
    swingh = raw.get("swingh")
    state = {
        "power": raw.get("power"),
        "mode": _MODE.get(mode, mode),
        "temp": raw.get("degrees"),
        "temperature": raw.get("degrees"),
        "fan": _FAN.get(fan, fan),
        "swingV": _SWING_V.get(swingv, swingv),
        "swingH": _SWING_H.get(swingh, swingh),
        "model": raw.get("model"),
    }
...
async def _native_rows(client: HanJooCoreClient, captures: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    results = await asyncio.gather(
        *(client.probe_all(list(cap.get("timings") or [])) for cap in captures),
    ...
    for index, (cap, result) in enumerate(zip(captures, results)):
        ...
        for match in matches:
            if not isinstance(match, dict) or not match.get("ac_state") or not match.get("protocol"):
                continue
    ...
    for protocol, hits in grouped.items():
        # One vote per physical press. Multiple decoder variants from one press
        ...
        # Guided 24/25/26 captures are a strong cross-check. For auto mode where
        # semantic expectations may be empty, require repeated protocol agreement
        # and at least two distinct decoded states.
        ...
        rows.append({
            "candidate": candidate,
            "group_id": f"protocol:irremoteesp8266:{protocol.lower()}",
            ...
            "evidence_sources": ["irremoteesp8266_native_ac"],
            "evidence": "native_ac_decode",
            "_core_recommended": valid >= 3 and (semantic >= 0.80 if guided else distinct >= 2),
        })
...
async def _fuse_with_native(self: HanJooCoreClient, captures: list[dict[str, Any]], *, kind_hint: str = "auto", candidates=None, profiles=None):
    # Keep Brain as the source of truth, then inject only native A/C evidence that
    # independently agrees across at least three physical presses.
    result = await _ORIGINAL_FUSE(
        self, captures, kind_hint=kind_hint, candidates=candidates, profiles=profiles
    )
    if str(kind_hint or "auto").lower() not in {"auto", "air_conditioner", "climate"}:
        return result
    native = await _native_rows(self, captures)
    if not native:
        # Prevent weak 2/3 A/C heuristics from generating misleading brand/model
        # suggestions when no native decoder corroborates them.
    ...
    existing = [row for row in existing if str((row.get("candidate") or {}).get("id") or "") not in native_ids]
    result["candidates"] = native + existing
    best = native[0]
    if bool(best.get("_core_recommended")):
        ...
        result["message"] = f"Native AC decoder confirmed {best['candidate']['model']} across {best['matched_captures']}/{best['capture_count']} captures."
